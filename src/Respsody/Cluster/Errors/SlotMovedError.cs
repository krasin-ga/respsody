...
public readonly ref struct SlotMovedError(ushort slot, ReadOnlySpan<char> endPoint)
{
    public ushort Slot { get; } = slot;
    public ReadOnlySpan<char> EndPoint { get; } = endPoint;

    public static bool TryParse(RespErrorResponseException exception, out SlotMovedError error)
    {
        ...
        if (count == 1)
        {
            error = new SlotMovedError(slot, []);
            return true;
        }

        error = new SlotMovedError(slot, remainingSpan[ranges[1]]);
        return true;
    }
}